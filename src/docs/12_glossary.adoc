...
| Flyway                      |                         | https://flywaydb.org/[Flyway] is a SQL centric tool for applying database migrations in a controlled fashion.
...
| jOOQ                        |                         | Java Object oriented querying: http://www.jooq.org/[jOOQ] is a framework for writing typesafe SQL with a fluent DSL from Java.
...
| SLF4J                       | Simple Logging Facade for Java | The http://www.slf4j.org[Simple Logging Facade for Java (SLF4J)] serves as a simple facade or abstraction for various logging frameworks (e.g. java.util.logging, logback, log4j) allowing the end user to plug in the desired logging framework at deployment time.
